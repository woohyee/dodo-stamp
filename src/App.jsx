    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const visitResponse = await fetch(`${API_URL}/api/visit`, {
                 method: 'POST',
                 headers: { 'Content-Type': 'application/json' },
                 body: JSON.stringify({ phone: result.phone_number, tag_id: 'DODO-T001-Test' }),
            });
            const visitResult = await visitResponse.json();
            if (!visitResponse.ok) throw new Error(visitResult.error || 'Failed to record first visit.');
    ...
    return (
        <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
            <div className="text-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800">Welcome to the Stamp Book!</h2>
                <p className="text-gray-500 mt-1">Register to collect your first stamp.</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
                <input type="text" placeholder="Name" value={name} onChange={e => setName(e.target.value)} required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                <input type="tel" placeholder="Phone Number" value={phone} onChange={e => setPhone(e.target.value)} required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <label htmlFor="agree" className="text-sm text-gray-600">I agree to the <a href="#" className="underline text-indigo-600">Privacy Policy</a>.</label>
                </div>
    ...
    return (
        <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-800">
                Welcome back, <span className="text-indigo-600">{result.customer.name}</span>!
            </h2>
            <p className="text-gray-500 mt-2 text-lg">
                Your visit has been successfully recorded!
            </p>
             <p className="mt-4 text-sm text-gray-500">
                Total Visits: <span className="font-bold text-3xl text-orange-500 mx-1">{result.totalVisits}</span>
            </p>
        </div>
    );
};
...
const VisitPage = () => {
    ...
    if (pageState === 'loading') {
        return <div className="text-center"><Loader /> <p className="mt-2">Verifying customer information...</p></div>;
    }
    ...
    return (
        <div className="font-sans min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4 space-y-8">
            <div className="w-full max-w-md p-4 bg-white rounded-2xl shadow-md">
                <h3 className="text-lg font-bold text-center mb-4">NFC Tag Scan Simulation</h3>
                ...
                <p className="text-center text-xs text-gray-500 mb-2">Press the button to simulate a new scan.</p>
                <div className="grid grid-cols-1">
                    <button onClick={resetSimulation} className="bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 transition">
